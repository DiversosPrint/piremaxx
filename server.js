...
app.delete('/api/data/:collection/:id', auth, masterOnly, async (req, res) => { try { if (!collections.includes(req.params.collection)) return res.status(400).json({ error: 'Coleção inválida.' }); await pool.query('DELETE FROM app_records WHERE collection=$1 AND record_id=$2',[req.params.collection,String(req.params.id)]); res.json({ ok: true }); } catch (error) { res.status(500).json({ error: error.message }); } });
